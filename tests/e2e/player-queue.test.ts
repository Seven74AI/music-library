 ...
 */
async function playTrackFromLibrary(
...
test.describe('Player / Queue', () => {
	...
	test('playing from playlist shows playlist context in queue', async ({
		...
		await testPrisma.servicePlaylistTrack.create({
			data: {
		...
		// Cleanup
		await testPrisma.servicePlaylistTrack.deleteMany({ where: { playlistId: playlist.id } })
		await testPrisma.servicePlaylist.delete({ where: { id: playlist.id } })
	})

	// ─────────────────────────────────────────────────
	// 2. Offline playback error message
	// ─────────────────────────────────────────────────
	test('handles track without audio file gracefully', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	// ─────────────────────────────────────────────────
	// 3. Add/remove tracks from Up Next queue
	// ─────────────────────────────────────────────────
	test('can add and remove tracks from Up Next queue', async ({
		page,
		login,
		insertNewTrack,
	}) => {
		...
		// Play track1 first
		await playTrackFromLibrary(page, 'UpNext Track 1')
	...
	// ─────────────────────────────────────────────────
	test('shuffle toggle reflects in player bar', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	// ─────────────────────────────────────────────────
	test('loop toggle cycles through off/all/one modes', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	test('shows queue-only bar when tracks are queued but not playing', async ({
		...
		// Play a track — the player bar should show with track info
		await playTrackFromLibrary(page, 'QueueOnly Track 1')
		await dismissInstallBanner(page)
	...
	// ─────────────────────────────────────────────────
	test('sleep timer starts and can be cancelled', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	// ─────────────────────────────────────────────────
	test('download button is visible in player bar', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	test('play/pause toggle works', async ({ page, login, insertNewTrack }) => {
		...
		await expect(playerBar.getByLabel('Play')).toBeVisible({ timeout: 5000 })

		// Click play → shows "Pause"
		await playerBar.getByLabel('Play').click()
	...
	// ─────────────────────────────────────────────────
	test('next and previous buttons are visible', async ({
		page,
		login,
		insertNewTrack,
	}) => {
		...
		await playTrackFromLibrary(page, 'NextPrev Track 1')
		await dismissInstallBanner(page)
	...
	// ─────────────────────────────────────────────────
	test('volume control and mute toggle are visible', async ({
		page,
		login,
		insertNewTrack,
	}) => {
	...
	// 14. Mobile mini bar
	// ─────────────────────────────────────────────────
	test('mobile mini bar is visible on small viewport', async ({
		page,
		login,
		insertNewTrack,
	}) => {
