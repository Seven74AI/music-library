...
test("Users can register and use passkeys", { tag: "@slow" }, async ({ page, navigate, login }) => {
  ...
  expect(initialCredentials.credentials, "No credentials should exist initially").toHaveLength(0);
  ...
  const afterRegistrationCredentials = await client.send("WebAuthn.getCredentials", {
    authenticatorId,
  });
  expect(
  ...
  expect(afterLoginCredentials.credentials[0]?.signCount).toBeGreaterThan(signCount1);
  ...
  const afterDeletionCredentials = await client.send("WebAuthn.getCredentials", {
    authenticatorId,
  });
...
test(
  "Failed passkey verification shows error",
  { tag: "@slow" },
