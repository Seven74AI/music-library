...
.youtube.com\tTRUE\t/\tFALSE\t0\tPREF\tf1=50000000`;
  ...
  test("shows error on invalid cookie content", async ({ page, loginAsAdmin }) => {
    ...
    await expect(page.getByText(/no valid cookies|cannot be processed/i)).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("shows error on empty file upload", async ({ page, loginAsAdmin }) => {
    ...
    // Should show error
    await expect(page.getByText(/empty|cannot be processed|no valid cookies/i)).toBeVisible({
      timeout: 10000,
    });
  });
