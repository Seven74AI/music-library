...
export class ServicePlaylistService {
	...
	private async processBatches(
		...
		for (let batchStart = 0; batchStart < totalItems; batchStart += TRANSACTION_BATCH_SIZE) {
			...
			try {
				...
				batchResult.processedExternalIds.forEach((id) =>
					accumulated.processedExternalIds.add(id),
				)
				batchResult.processedTrackIds.forEach((id) =>
					accumulated.processedTrackIds.add(id),
				)
	...
	async getAllPlaylistsWithSyncStatus(
		...
		try {
			...
			const syncedPlaylistInternalIds = new Map(
				syncedPlaylists.map((p) => [p.externalId, p.id]),
			)

			const playlistsWithSyncStatus: PlaylistWithSyncStatus[] = allPlaylists.map(
				(playlist) => ({
					...playlist,
	...
	private async registerAndSyncPlaylist(
		...
		const playlistData = syncProvider.normalizePlaylistData(
			rawPlaylist,
			service.id,
			userId,
		)
