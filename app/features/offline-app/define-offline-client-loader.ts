...
export function defineOfflineClientLoader<TOnline, TOffline>(routeId: string) {
  ...
  async function clientLoader(args: OfflineClientLoaderArgs): Promise<TOnline | TOffline> {
    if (!cachedLoader) {
      const { createOfflineClientLoader } =
        await import("#app/features/offline-app/offline-loader.client.ts");
...
export function defineDeviceOnlyClientLoader<TOffline>(routeId: string) {
  ...
  async function clientLoader(): Promise<TOffline> {
    if (!cachedLoader) {
      const { createDeviceOnlyClientLoader } =
        await import("#app/features/offline-app/offline-loader.client.ts");
