...
export const BreadcrumbHandle = z.object({
  breadcrumb: z.union([z.custom<React.ReactNode>(), z.custom<BreadcrumbFn>()]),
});
  ...
  const breadcrumbs = matches
    .map((m) => {
      ...
      const breadcrumbContent =
        typeof breadcrumb === "function" ? breadcrumb({ loaderData: m.loaderData }) : breadcrumb;
          ...
          <BreadcrumbItem key={breadcrumb.key}>
            ...
            {i === arr.length - 1 ? (
              ...
              <BreadcrumbLink asChild>
                <Link to={breadcrumb.pathname}>{breadcrumb.content}</Link>
              </BreadcrumbLink>
            )}
          </BreadcrumbItem>
        ))}
