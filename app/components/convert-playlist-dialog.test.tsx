...
const mockConvertFetcher = {
  state: "idle" as const,
  data: undefined as { status: string; message?: string; existingTitle?: string } | undefined,
...
function renderDialog() {
  const router = createMemoryRouter(
    [
      {
        path: "/",
        element: <ConvertPlaylistDialog playlistId="sp-1" playlistTitle="Chill Vibes" />,
      },
    ],
    { initialEntries: ["/"] },
  );
...
test("opens menu with create and add options", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "More actions for Chill Vibes" }));
...
test("add mode submits on playlist click", async () => {
  const user = userEvent.setup();
  mockPlaylistsFetcher.data = {
    playlists: [{ id: "up-1", title: "My Mix", description: null, _count: { tracks: 12 } }],
  };
...
test("shows duplicate title error in create mode", async () => {
  ...
  expect(screen.getByText('You already have a playlist named "Chill Vibes"')).toBeDefined();
});
