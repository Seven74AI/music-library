...
export function DeletedVideoMatchDialog({
  ...
  const [selections, setSelections] = useState<
    Record<number, { action: "match" | "new" | "skip"; trackId?: string }>
  >({});
    ...
    <Dialog open={true} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Some videos in this playlist have been deleted from YouTube. Please choose how to handle
            each one.
          </DialogDescription>
        </DialogHeader>
            ...
            return (
              <div key={matchIndex} className="border rounded-lg p-4 space-y-3">
                ...
                {match.deletedVideo.title && (
                  <p className="text-sm text-muted-foreground">{match.deletedVideo.title}</p>
                )}
                ...
                {match.candidateTracks.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    <RadioGroup
                      value={
                        selection?.action === "match"
                          ? `match-${selection.trackId}`
                          : selection?.action || ""
                      }
        ...
        <DialogFooter>
          ...
          <Form method="post" action={`/music/services/youtube/playlist/${playlistId}`}>
            <input type="hidden" name="intent" value="confirm-deleted-match" />
            <input
              type="hidden"
              name="matches"
              value={JSON.stringify(
                pendingMatches.map((match, index) => {
                  ...
                  return {
                    deletedItemId: match.deletedVideo.itemId,
                    selectedTrackId:
                      selection.action === "match" ? selection.trackId || null : null,
                    position: match.deletedVideo.position,
