...
let refreshFetcherData: { notifications: NotificationItem[]; unreadCount: number } | undefined;
...
vi.mock("react-router", async (importOriginal) => {
  ...
  return {
    ...
    useFetcher: vi.fn().mockImplementation(() => {
      ...
      return fetcherCallCount % 2 === 1 ? mockSubmitFetcher : mockRefreshFetcher;
    }),
  };
});
...
test("removes loading state when fetcher returns to idle", () => {
  ...
  expect(screen.getByRole("button", { name: "Processing notifications..." })).toBeDefined();
