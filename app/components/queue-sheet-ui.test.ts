...
describe('formatQueueSheetTitle', () => {
	...
	test('shows only up next when spine total is zero', () => {
		expect(formatQueueSheetTitle(2, 0, 'from library')).toBe(
			'Queue (2 up next)',
		)
	})

	test('shows only spine when up next is empty', () => {
		expect(formatQueueSheetTitle(0, 500, 'from playlist')).toBe(
			'Queue (500 from playlist)',
		)
	})
...
describe('getSpineSectionLabel', () => {
	...
	test('uses playlist label for playlist context', () => {
		expect(
			getSpineSectionLabel({ type: 'playlist', playlistId: 'p1' }),
		).toBe('from playlist')
	})
...
describe('getSpineSectionHeading', () => {
	...
	test('uses playlist heading for playlist context', () => {
		expect(
			getSpineSectionHeading({ type: 'playlist', playlistId: 'p1' }),
		).toBe('From Playlist')
	})
})
