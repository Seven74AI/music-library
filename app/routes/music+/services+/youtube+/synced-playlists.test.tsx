...
vi.mock("#app/utils/error-handlers.server", () => ({
  handleLoaderError: vi.fn((_error: unknown, fallback: unknown, _context: string) => ({
    data: fallback,
  })),
}));
...
describe("synced-playlists loader", () => {
  ...
  test("returns playlists with track counts", async () => {
    ...
    vi.mocked(createServicePlaylistService).mockReturnValue(mockService as any);
    ...
    expect(mockService.getSyncedPlaylists).toHaveBeenCalledWith("youtube", "user-1");
  });
