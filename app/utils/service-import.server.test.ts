...
describe("extractYouTubeVideoId", () => {
  it("returns the ID for plain 11-char video IDs", async () => {
    const { extractYouTubeVideoId } = await import("./service-import.server.ts");
...
describe("importTrackDirectly", () => {
  beforeEach(async () => {
    ...
    // Reset mocks
    const { getYouTubeVideoDetails } = await import("#app/utils/youtube-search.server.ts");
    const { getServiceByName } =
      await import("#app/features/service-playlist/service-playlist.server.ts");
    const { getOrCreateArtistTx } = await import("#app/utils/artist-management.server.ts");
    const { enqueueArchiveJob } =
      await import("#app/features/audio-archive/auto-enqueue.server.ts");
    ...
    mockPrisma.$transaction.mockImplementation(async (fn: (tx: any) => any) => fn(mockTx));
  });

  describe("successful import (new track)", () => {
    it("returns success with trackId when creating a new track", async () => {
      const { importTrackDirectly } = await import("./service-import.server.ts");
    ...
    it("accepts a full YouTube URL", async () => {
      ...
      const result = await importTrackDirectly("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
    ...
    it("maps publishedAt to releaseDate on the created track", async () => {
      ...
      if (upsertCall?.create?.releaseDate) {
        ...
        expect(upsertCall.create.releaseDate.toISOString()).toBe("2009-10-25T06:57:33.000Z");
      }
    });
  });
  ...
  describe("error handling", () => {
    ...
    it("returns error when YouTube API fails", async () => {
      const { getYouTubeVideoDetails } = await import("#app/utils/youtube-search.server.ts");
      vi.mocked(getYouTubeVideoDetails).mockRejectedValue(new Error("YouTube quota exceeded"));
    ...
    it("returns error when service is not found in database", async () => {
      const { getServiceByName } =
        await import("#app/features/service-playlist/service-playlist.server.ts");
      vi.mocked(getServiceByName).mockRejectedValue(new Error("Service not found: youtube"));
