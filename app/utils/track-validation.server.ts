...
export const SpotifyTrackValidationSchema = TrackValidationSchema.extend({
  serviceProviderId: z.string().regex(/^[a-zA-Z0-9]{22}$/, "Invalid Spotify track ID format"),
  serviceUrl: z
    .string()
    .regex(/^https:\/\/open\.spotify\.com\/track\/[a-zA-Z0-9]{22}$/, "Invalid Spotify URL format"),
});
