...
export function buildMediaSessionMetadata(track: FullTrack) {
  return {
    ...
    album: "Unknown Album",
    artwork: track.coverImage ? buildMediaSessionArtwork(track.coverImage.objectKey) : [],
  };
}
...
export function isMediaSessionPositionStateSupported(): boolean {
  return isMediaSessionSupported() && "setPositionState" in navigator.mediaSession;
}

type MediaSessionTiming = Pick<HTMLAudioElement, "currentTime" | "duration" | "playbackRate">;

export function clampMediaSessionSeekTime(seekTime: number, duration: number): number {
