...
import { YOUTUBE_RATE_LIMITS, YOUTUBE_API_VERSION } from "#app/config/youtube";
...
import { type VideoData } from "#app/utils/youtube-utils";
...
export async function searchYouTubeVideos(
  query: string,
  maxResults = YOUTUBE_RATE_LIMITS.DEFAULT_SEARCH_RESULTS,
): Promise<VideoData[]> {
  // Input validation
  ...
  if (
    maxResults < YOUTUBE_RATE_LIMITS.MIN_SEARCH_RESULTS ||
    maxResults > YOUTUBE_RATE_LIMITS.MAX_SEARCH_RESULTS
  ) {
    throw new YouTubeValidationError(
  ...
  try {
    ...
    const searchData = validateYouTubeAPIResponse(response.data, YouTubeSearchResponseSchema);
    ...
    throw new YouTubeAPIError(
      ...
      YOUTUBE_ERROR_CODES.SEARCH_FAILED,
      500,
    );
  }
}
...
export async function getYouTubeVideoDetails(videoId: string): Promise<VideoData> {
  ...
  if (!videoId || typeof videoId !== "string" || videoId.trim().length === 0) {
    throw new YouTubeValidationError(
      "Video ID is required and must be a non-empty string",
      "videoId",
    );
  }
  ...
  try {
    ...
    // Validate API response with new type-safe architecture
    const videoData = validateYouTubeAPIResponse(response.data, YouTubeVideoListResponseSchema);
    ...
    throw new YouTubeAPIError(
      "Failed to get YouTube video details. Please try again.",
