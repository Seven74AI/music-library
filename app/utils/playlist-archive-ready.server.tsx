...
export async function isServicePlaylistArchiveReady(playlistId: string): Promise<boolean> {
...
function resolveSiteOrigin(origin?: string): string {
  const raw = origin?.trim() || process.env.SITE_URL?.trim() || "http://localhost:3000";
  return raw.replace(/\/$/, "");
}
