...
25. **Library track row → user playlist (Spotify-like)** — When `TrackListItem` receives a `playlists` prop (including an empty `[]`), show **Add to Playlist** on library and user-playlist pages. `AddToPlaylistMenu` lists existing playlists (searchable) and always offers **Create new playlist** → `/playlists/new?trackId=…`. Adding to an existing playlist POSTs to `/resources/add-track-to-playlist` (duplicate detection with confirm). Creating from a track: `/playlists/new` accepts optional `trackId` (loader + hidden form field); on submit, create `UserPlaylist`, add the track via `addTrackToUserPlaylist()` in `app/utils/user-playlist.server.ts`, then redirect to `/library` with a success toast. Standalone `/playlists/new` (no `trackId`) still redirects to `/playlists/{id}`. Do not gate the menu on `playlists.length > 0` — new users with zero playlists must still reach create-from-track.
